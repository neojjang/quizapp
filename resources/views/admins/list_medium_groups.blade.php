        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
            <div class="mx-auto">
                <div class="flex justify-between items-center py-4">
                    <a href="{{route('createMediumGroup')}}" class="tracking-wide font-bold rounded border-2 border-blue-500 hover:border-blue-500 bg-blue-500 text-white hover:bg-blue-600 transition shadow-md py-2 px-6 items-center">수업 중분류 생성</a>
                                        ...
                                        <tr class="hover:bg-green-100">
                                            <td class="px-6 ">
                                                <div class="flex items-center">
                                                    @if(isset($section->major_group))
                                                    <div class="ml-4">
                                                        <div class="text-sm font-medium text-gray-900">
                                                            <a class="text-blue-400 hover:underline" href="{{ route('detailMajorGroup', $section->major_group->id) }}">
                                                                {{ $section->major_group->name}}
                                                            </a>
                                                        </div>
                                                    </div>
                                                    @endif
                                            ...
                                            <td class="px-6 py-1">
                                                <div class="text-sm text-gray-900">{{ $section->class_rooms_count }}</div>
                                            </td>
                                            <td class="sm:flex align-middle justify-center items-center px-6 py-1 text-right text-sm font-medium">
                                                ...
                                                <form action="{{route('deleteMediumGroup',$section->id)}}" method="post" onsubmit="return deleteClassRoom('{{ $section->name}}'); return false;">
                                                    @csrf
    ...
    @push('js')
        <script type="text/javascript">
            function deleteClassRoom(title) {
                return confirm(`[${title}] 수업 중분류를 삭제 하시겠습니까?`);
            }
        </script>
    @endpush
</x-app-layout>
